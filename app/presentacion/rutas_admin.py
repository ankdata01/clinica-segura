...
def crear_router(plantillas) -> APIRouter:
    ...
    # ------------------------------------------------------------------ #
    # POST /admin/nuevo — crear usuario y mostrar QR                        #
    # ------------------------------------------------------------------ #
    ...
    async def crear_usuario(
        ...
        # Par RSA (necesario por la restricción NOT NULL de llave_publica)
        ...
        con.execute(
            """INSERT INTO personal
               ...
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, 1)""",
            (
                uid, nombre, rol, especialidad, email,
                _ph.hash(pass_temp), mfa_secret, serializar_publica(pub),
            ),
        )
        con.commit()

        _auditar(
            RepoAuditoria(con), usuario["sub"],
            "CREAR_USUARIO",
            f"email={email} rol={rol}",
            _ip(request),
        )

        csrf = generar_csrf()
        resp = plantillas.TemplateResponse(
            request, "admin_qr.html",
            {
                "usuario":   usuario,
                "nuevo":     {"nombre": nombre, "email": email, "rol": rol},
                "pass_temp": pass_temp,
                "totp_uri":  totp_uri,
                "qr_data":   qr_data,
                "csrf_token": csrf,
    ...
    # ------------------------------------------------------------------ #
    # POST /admin/usuarios/{uid}/toggle — activa/desactiva                 #
    # ------------------------------------------------------------------ #
    ...
    # ------------------------------------------------------------------ #
    # POST /admin/usuarios/{uid}/eliminar — baja definitiva                 #
    # Solo permitido si el usuario ya está inactivo (suspendido).           #
    # ------------------------------------------------------------------ #
    ...
    async def eliminar_usuario(
        ...
        fila = con.execute(
            "SELECT activo, email, nombre FROM personal WHERE id = ?", (uid,)
        ).fetchone()

        if not fila:
            return RedirectResponse(url="/admin", status_code=303)

        # Solo se puede eliminar si ya está suspendido
        if fila["activo"]:
            return RedirectResponse(url="/admin?error=suspender_primero", status_code=303)

        # No eliminarse a uno mismo
        if uid == usuario["sub"]:
            return RedirectResponse(url="/admin", status_code=303)

        # No se puede eliminar si tiene entradas en audit_log (FK constraint).
        # En ese caso se deja inactivo de forma permanente — la trazabilidad lo requiere.
        entradas_audit = con.execute(
            "SELECT COUNT(*) FROM audit_log WHERE personal_id = ?", (uid,)
        ).fetchone()[0]
        if entradas_audit > 0:
        ...
        _auditar(
            RepoAuditoria(con), usuario["sub"],
            "ELIMINAR_USUARIO",
            f"email={fila['email']} nombre={fila['nombre']}",
            _ip(request),
        )
